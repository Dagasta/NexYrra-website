...
import { Terminal, Send, CheckCircle, Radio, Globe, Zap, MousePointer2 } from 'lucide-react';
                ...
                <div className="container-nex">
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 140 }} className="grid-mobile-1">
                        ...
                        <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }}>
                            ...
                            <h1 className="text-bionic" style={{ fontSize: 'clamp(50px, 12vw, 150px)', fontWeight: 800, lineHeight: 0.85, letterSpacing: '-0.07em', marginBottom: 60 }}>
                                SECURE <br />
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 60 }}>
                                {[
                                    ...
                                    <div key={i} style={{ display: 'flex', gap: 30, alignItems: 'center' }}>
                                        <div style={{ width: 60, height: 60, background: 'rgba(255,255,255,0.03)', borderRadius: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(255,255,255,0.05)' }}>
                                            <item.icon size={22} style={{ color: '#8B5CF6' }} />
                                        </div>
                                        <div>
                                            <div className="font-cyber" style={{ fontSize: 9, color: '#1E293B', marginBottom: 8 }}>{item.l}</div>
                                            <div className="font-title" style={{ fontSize: 24, fontWeight: 800 }}>{item.v}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {/* Portal Entry Portal: The Form */}
                        <div className="glass-prismatic wow-card" style={{ padding: '80px' }}>
                            <AnimatePresence mode="wait">
                                {status === 'success' ? (
                                    <motion.div key="success" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: 'center', padding: '60px 0' }}>
                                        ...
                                        <h3 className="font-title" style={{ fontSize: 36, fontWeight: 800, marginBottom: 20 }}>STATION_SYNCED.</h3>
                                        <div className="font-cyber" style={{ fontSize: 10, color: '#4B5563' }}>Neural-packet received. Coordination sequence scheduled: POS_01.</div>
                                    </motion.div>
                                ) : (
                                    <motion.form key="form" onSubmit={handleSubmit} layout style={{ display: 'grid', gap: 60 }}>
                                        <div className="font-cyber" style={{ fontSize: 10, color: '#1E293B' }}>[ NEURAL_PARAMETER_INPUT ]</div>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40 }} className="grid-mobile-1">
                                            <div style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: 15 }}>
                                                <label className="font-cyber" style={{ display: 'block', marginBottom: 15, fontSize: 9 }}>ID_SIGNATURE</label>
                                            ...
                                            <div style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: 15 }}>
                                                <label className="font-cyber" style={{ display: 'block', marginBottom: 15, fontSize: 9 }}>ID_UPLINK_MAIL</label>
                                        ...
                                        <div style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: 15 }}>
                                            <label className="font-cyber" style={{ display: 'block', marginBottom: 15, fontSize: 9 }}>SYSTEM_OBJECTIVES</label>
                                        ...
                                        <button type="submit" className="btn-beyond" disabled={status === 'loading'} style={{ borderRadius: '12px', justifyContent: 'center' }}>
